...
# User Configuration
# ------------------
START_FREQ       = 1000     # Highest frequency in Hz
END_FREQ         = 1        # Lowest frequency in Hz
NUM_STEPS        = 60       # Number of steps in the log sweep
NOTE_DURATION_MS = 100      # Each frequency lasts this many ms
PIO_PIN          = 15       # Pin for output (speaker/buzzer)

CLK_FREQ = 125_000_000      # The default system clock for PIO on Pico

# ---------------------
# Calculate Log Spacing
# ---------------------
log_start = math.log(START_FREQ)
log_end   = math.log(END_FREQ)
log_step  = (log_end - log_start) / NUM_STEPS
frequencies = [math.exp(log_start + i * log_step) for i in range(NUM_STEPS + 1)]

# -----------------
# PIO Assembly Code
# -----------------
@rp2.asm_pio(
    out_init=(rp2.PIO.OUT_LOW,),  # The pin starts LOW
    autopull=True,
    pull_thresh=32
)
def square_wave():
    # Read 32-bit half-period from FIFO -> X
    # Toggle pin HIGH, decrement X, then toggle pin LOW, decrement X again
    wrap_target()

    mov(x, osr)        # x := OSR
    set(pins, 1)       # pin -> high
    label("delay_high")
    jmp(x_dec, "delay_high")  # decrement x until zero
...
# -----------------------
pio = rp2.PIO(0)
sm = rp2.StateMachine(0, square_wave, freq=CLK_FREQ, out_base=Pin(PIO_PIN))
    ...
    try:
        while True:
            for freq in frequencies:
                if freq < 1:
                    freq = 1  # clamp to 1 Hz if anything goes under
                half_period = int(CLK_FREQ / (2 * freq))
                sm.put(half_period)
                time.sleep_ms(NOTE_DURATION_MS)
            # After finishing the frequency list, just loop back to the beginning
